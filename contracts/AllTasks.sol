...
contract TasksContract {
    ...
    event Log(uint _amount, uint gas)
    constructor() {
        // console.log('Owner contract deployed by: %s', msg.sender);
    }

    function createTask(uint256 _reward, uint256 _deadline) external payable {
        require(_reward > 0, 'Reward must be greater than zero');
        require(msg.value >= _reward, 'Insufficient balance to cover the reward');
        ...
        (bool success, ) = payable(address(this)).call{value: _reward}(
            ""
        );
    ...
    function cancelTask(uint256 _taskIndex) external payable {
        Task storage _task = tasks[_taskIndex];
        require(
            msg.sender == _task.owner,
